...
namespace Leguar.TotalJSON
{
    ...
    public class JSON : JValue
    {
        ...
        {
            JValue jValue;
            if (keyValuePairs.TryGetValue(key, out jValue))
            {
        ...
        internal override void zCreate(CreateStringRunner createStringRunner)
        {
            ...
            {
                DebugLogger.LogInternalError("JSON.zCreate(): 'keyValuePairs.Count' (" + keyValuePairs.Count + ") is no equal to 'keyOrder.Count' (" + keyOrder.Count + ")");
            }
            if (keyValuePairs.Count > 0)
            {
